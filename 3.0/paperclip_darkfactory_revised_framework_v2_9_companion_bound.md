...
- 传播 `artifact_certification_changed`、`lineage_invalidation_started`、`lineage_quarantine_required`。  
- 为下游消费者暴露最小可消费认证视图。  
...
1. 发出 `artifact_certification_changed` 事件；  
2. 建立 `lineage_invalidation_started` 记录；  
...
- `lineage_quarantine_required`  
- `profile_conformance_changed`  
- `capsule_health_changed`  
...
- waiver 不得抑制 `artifact_certification_changed` / `lineage_invalidation_*` 事件。  
...
3. unknown-field round-trip 与 write-fence 测试通过；  
4. `state_transition_matrix.csv` 覆盖正文要求的关键迁移与非法组合；  
5. 任何 impl 包中的示例，不得把 `reopened`、`revoked`、`unverifiable`、`broker_bypass_attempted`、`manual_*` 等状态弱化为“warning only”。  
