...
默认规则：
- `setup_timeout_seconds: 180`：覆盖容器启动、挂载缓存、依赖安装、预处理
- `boot_timeout_seconds: 30`：仅覆盖 Preview Server 进程启动并监听端口
- setup 超时：判定为 `PREVIEW_SETUP_TIMEOUT`，错误类别为 `environment_error`
- boot 超时：判定为 `PREVIEW_BOOT_TIMEOUT`
- 已启动但健康检查失败：判定为 `preview_unhealthy`

**重要约束**：`PREVIEW_SETUP_TIMEOUT` 不进入代码修复回流链，必须优先走环境重试、无缓存重试、切换预热镜像或人工排查。
...
Agent **不得自由拼接** `git checkout <dynamic-branch>` 或 `git switch <dynamic-branch>`。
执行层必须提供受控工具方法：
...
这些工具内部只能操作 `task/<task_id>` 命名规范的分支，禁止切换到 `main`、`master`、`develop` 或其他任务分支。
- 验证通过后生成 PR
- 默认采用 `Squash Merge`
...
> `resume_base_commit` 不由前端填写，必须由后端在收到 Resume 请求后自动 `fetch` 远端 `task/<task_id>` 分支 HEAD 并写入。
...
- Handoff 时记录 `handoff_commit`
- Resume 时后端自动抓取 `resume_base_commit`
- 若 `resume_base_commit == handoff_commit` 且 `resume_mode == resume_from_failed_gate`，系统必须自动降级为 `resume_from_replan`，避免在完全相同代码状态上死循环重试
- Handoff 后恢复执行必须重建 worktree 与容器，禁止复用旧 worktree 继续跑

---

# 16.5 任务状态机

以下状态机为前后端、调度器和控制平面的单一事实来源：
...
stateDiagram-v2
    ...
    queued --> planning
    planning --> running
    planning --> paused_due_to_model_unavailable
    planning --> budget_exceeded
    planning --> handoff_required
    ...
    awaiting_scope_approval --> queued: approved
    awaiting_scope_approval --> handoff_required: rejected / timeout
    handoff_required --> manual_in_progress
    manual_in_progress --> resume_pending
    manual_in_progress --> cancelled
    ...
    aborted --> queued: manual retry allowed
    cancelled --> [*]
    completed --> [*]
```

状态说明：
- `awaiting_scope_approval`：等待人工审批范围变更，且不持有 Writer Lock
- `handoff_required`：需要人工接管
- `resume_pending`：人工完成，等待系统恢复
- `budget_exceeded`：预算硬限制触发
- `paused_due_to_model_unavailable`：关键模型不可用导致暂停
...
禁止：
- 任意 `pnpm *` 宽泛通配符
- 任意 `go test *` / `go build *` 宽泛通配符
- 任意 `playwright test *` 宽泛通配符
- `curl *`
- `wget *`
- `nc *`
- `rm -rf *`
...
本文件为 v1.3.3 TRUE MERGED 单一事实来源版本。正文已吸收此前补丁中的安全、状态机、恢复、超时、预算与 API 修订；不再依赖附录优先级解释。
