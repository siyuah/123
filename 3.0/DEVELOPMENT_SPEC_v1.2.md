...
preview:
  ...
  port_pool_end: 41999
  docker_network_prefix: df_task_
```


## 7.5 Preview Networking Strategy

第一阶段优先采用 **单容器模式**，避免引入不必要的 Docker 网络复杂度。

### 单容器模式（默认）

- Preview Server 与 Playwright 运行在同一个 task 容器中
- Playwright 固定访问：
  - `http://127.0.0.1:<allocated_port>`
- 不需要额外 service discovery
- 更适合 MVP 与高并发 task 环境

### 双容器模式（仅在确有需要时启用）

如果项目必须将 Preview Server 与 Playwright 分离，则必须：

1. 为每个 task 创建独立 Docker Network：
   - `df_task_<task_id>`
2. Preview 容器固定 service name：
   - `preview`
3. Playwright 容器仅允许访问：
   - `http://preview:<port>`
4. Control Plane 负责：
   - 创建网络
   - 启动 preview 容器
...
### Service Discovery 规则

- 单容器模式：不需要 service discovery
- 双容器模式：仅允许使用 Docker 内建 DNS 解析 service name
- 禁止依赖宿主机端口映射来做 task 内部互联
- 禁止多个 task 共享同一个 preview network

## 7.6 Preview 生命周期

Preview 环境必须有明确生命周期：

1. allocate port / network
2. start preview
3. health check until ready
4. run Playwright
5. collect artifacts
6. stop preview
7. cleanup container / network / temp files

任何一个步骤失败，都必须进入统一清理流程。
...
- 健康检查至少支持：
  ...
  - 可选项目自定义 `/healthz`
- 需要区分：
  - preview_boot_timeout_sec：服务启动超时
  - preview_ready_timeout_sec：健康检查等待超时
- 超时后：
  - 写入 verification failure
  - 保存 stderr / logs
  - 执行 cleanup
...
## 8.4 Worktree 生命周期与清理

每个 Task 的 worktree 必须有完整生命周期管理：
...
该模块至少负责：
- `createWorktree(taskId, baseCommit)`
...
所有执行流程必须使用统一的 `try/finally` 清理模式：
...
此外还必须有：
- `on_exit` 钩子
- daemon 启动后的 orphaned worktree 扫描
- 周期性 `git worktree prune`

## 8.6 清理失败的兜底策略

若清理失败：

- 将 task 标记为 `cleanup_pending`
- 记录未清理路径
- 进入后台清理队列重试
- 超过阈值后告警
...
## 13.7 Real-Time Budget Interceptor

预算控制不能在任务结束后再结算，必须在模型调用过程中实时生效。
...
2. 流式过程中累计：
   ...
   - 估算成本
3. 触发预算阈值：
   - 80%：soft limit，禁止升级到更高成本模型
   - 100%：hard limit，停止新调用并将 task 标记为 `budget_exceeded`
4. 调用后记账：
   - 写回 `budget_used_usd`
   - 写回 token usage
   - 写回调用明细
...
Rate limiting 不能只停留在规则层，必须有实际队列实现。

要求：

- 按模型维护并发池
- 按 API key 维护 RPM / TPM 计数器
- 超限任务进入队列等待
- 高优先级任务允许抢占低优先级队列
- 当预算接近上限时，自动降级到低成本模型
...
## 16.7 Dependency Cache Strategy

为避免每个 Task 都全量安装依赖导致系统过慢，必须增加缓存策略。
...
这些缓存仅用于加速依赖获取，不得存储 task 私有状态。

### Task 私有层

必须独占：
- git worktree
- node_modules（如项目选择不共享）
- build output
- preview output
- artifacts
...
- 默认使用：
  - `pnpm install --ignore-scripts`
- 容器允许挂载：
  - 共享只读或受控写入的 PNPM Store volume
- 任务完成后不保留 task 私有依赖目录
...
- 共享缓存不等于共享工作目录
- 禁止跨 task 共享 `.env`、build artifacts、临时源码修改
- 缓存损坏时允许回退到无缓存安装模式
...
- Preview Networking Strategy
- Worktree Lifecycle and Cleanup
- Real-Time Budget Interceptor
- Dependency Cache Strategy

实现时必须优先采用：
- 单容器 Preview 模式
- `try/finally + on_exit` 清理
- 模型调用预算拦截器
- 共享 PNPM Store / Go cache / Playwright cache
